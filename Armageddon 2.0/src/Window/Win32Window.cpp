...
Armageddon::Win32Window::Win32Window(int width, int height, std::wstring title, std::wstring wclass) : w_height(height), w_width(width), w_title(title), w_class(wclass)
{
	...
	{
		Armageddon::Log::GetLogger()->trace("CreateWindowEx a �chou�");
...
LRESULT Armageddon::Win32Window::WindowProc(HWND hwnd, UINT uMsg, WPARAM wparam, LPARAM lparam)
{
	...
	case WM_CHAR:
	{
		...
		unsigned char letter = static_cast<unsigned char>(wparam); // on r�cup�re la lettre via le param Wparam
...
bool Armageddon::Win32Window::ProcessMessage()
{
...
void Armageddon::Win32Window::RegisterWindowClass()
{
