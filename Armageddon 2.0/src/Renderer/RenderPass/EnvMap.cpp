...
Armageddon::EnvMap::EnvMap(const std::filesystem::path& HDRPath)
{
	m_Cube = Armageddon::Primitives::GenerateCube();	
 

	m_Cube.v_MaterialReference[0] = AssetManager::GetOrCreateMaterial("EnvmapMaterial");
	AssetManager::m_MaterialMap[m_Cube.v_MaterialReference[0]].SetPixelShader(L"Assets/Shaders/SkyBoxPixel.cso");
	AssetManager::m_MaterialMap[m_Cube.v_MaterialReference[0]].SetVertexShader(L"Assets/Shaders/SkyBoxVertex.cso");
	m_envMapTexture.CreateCubeMap(HDRPath);
	m_convEnvMapTexture.CreateIrradiancedMap(m_envMapTexture.GetRessourceViewPtr());
	m_BRFLutTexture.Create(L"Assets/Texture/Skybox/ibl_brdf_lut.png");//TODO JE SUIS DEPITE
...
void Armageddon::EnvMap::Render(Armageddon::Camera* m_camera)
{
	
	Armageddon::Interface::GetDeviceContext()->RSSetState(rasterizerState.Get());

	CameraBuffer_t m_TransForm;
	...
	Armageddon::Renderer::g_CameraCBuffer.SetDynamicData(&m_TransForm);
	Armageddon::Renderer::g_CameraCBuffer.BindPS();
	Armageddon::Renderer::g_CameraCBuffer.BindVS();
	
	Armageddon::Interface::GetDeviceContext()->PSSetShaderResources(17,1, m_convEnvMapTexture.GetRessourceViewPtr());
